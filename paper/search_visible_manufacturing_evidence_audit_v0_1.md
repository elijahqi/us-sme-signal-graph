...
**Version:** Exploratory Working Paper v0.3
...
We contribute a documented corpus construction and support rubric, a reconstruction of consequential processing corrections, and an account of what those corrections do and do not validate. The revisions examined here were prompted by failures found during the study. They are retrospective process checks, not a preregistered comparison or a controlled accuracy experiment. Version 0.2 narrowed the earlier manuscript's emphasis after the results were known; v0.3 adds routing denominators and output-consistency checks using the same stored decisions. The original protocols, deviations, and outstanding validation requirements remain part of the record.
...
Every pair with a disputed key field must have a C record; absent adjudication is an execution failure. This rule preserves agreed fields but can combine decisions from different passes into a hybrid record. We therefore distinguish agreement preservation from consistency across output fields. Neither establishes factual correctness.
...
Table: Primary model-assigned reasons; denominator 5,016. The last row groups the remaining recorded categories. Percentages are rounded.
