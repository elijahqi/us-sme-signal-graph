...
The separate `scripts/run_glm_quality_review.py` is a bounded editorial/code-review trial through the supported Claude Code integration. It must not be used as a batch annotation pipeline on a Coding Plan; see Z.ai's [usage policy](https://docs.z.ai/devpack/usage-policy) and [supported-tool integration](https://docs.z.ai/devpack/tool/claude). It shares no research labels with the comparison above, enforces concurrency one for this project, and refuses silent reruns of an attempted request. Other clients using the same endpoint are outside this project lock.
