...
Private row-level files and fetched page bodies are excluded from public artifacts. A row-free exploratory aggregate package is published at `release/public/long-tail-evidence-audit-v0.1-exploratory`; its fail-closed validator excludes company rows, URLs, page text, quotes, rationales, and provider payload content. The existing `release/public/v0.2.0-rc1` is an earlier semiconductor pilot. The aggregate package was archived on GitHub in commit `c036829`, and all eight published SHA-256 entries were verified against the remote files. Public aggregate availability does not provide access to the closed row-level evidence.
